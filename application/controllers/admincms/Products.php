<?php
	...
	class Products extends CI_Controller
	{
		...
		function add_product_section()
		{
			...
			{
				//echo "errors";
				$data['site_title'] = "Add Products Section";
				$this->load->view('admincms/products/add_product_section', $data);
			}
			else
			{
				...
				if ( ! $this->upload->do_upload())
				{
					...
					$this->load->view('admincms/services/add_services_section', $error);
				}else{
		...
		function add_product()
		{
			...
			if($this->form_validation->run() == false)
			{
				...
				$this->load->view('admincms/products/add_product', $data);
			}
			else
			{
				$show_on_silder="";
				...
				if ( ! $this->upload->do_upload())
				{
					...
					$this->load->view('admincms/products/add_product', $error);
				}else{
				...
				$this->session->set_flashdata('message', '<p class=\'success\'>Products was successfully added.</p>');
				redirect(current_url());
			}
			}
			}
		} // end function add_services
		...
		{	
			...
			$this->db->delete('post_section', array('section_id' => $section_id)); 
			
			$this->session->set_flashdata('user_updated', '<p class=\'success\'Products Section was successfully deleted</p>');
			redirect('admincms/products/view_products');
		...
		{	
			...
			$this->db->delete('products', array('post_id' => $post_id)); 
		...
		function view_products_section()
		{	
			
			$data['site_title'] = "View Post";	
			$data['post'] = $this->data_model->ProductsSection();
			$this->load->view('admincms/products/view_products_section', $data);

			} // end function view_services
			
			
			function update_section_arrange()
		{
					if($this->input->post('sbm') == "Update Arrange") { 
			
							$arrange=$_POST['arrange'];
						foreach($arrange as $section_id => $value2)
						  {
						 ...
						 foreach($contentid as $id => $value2)
						  {
							 	...
							 	$this->db->delete('post_section', array('section_id' => $value2)); 		
								 }
						$this->session->set_flashdata('user_updated', '<p class=\'success\'>Sections was successfully deleted.</p>');
						redirect("admincms/products/view_products_section");
					}
		} // end function update_post_arrange
                
                function update_products_arrange()
		{
					if($this->input->post('sbm') == "Update Arrange") { 
						 ...
						 foreach($contentid as $id => $value2)
						  {
							 	...
							 	$this->db->delete('products', array('post_id' => $value2)); 		
								 }
						$this->session->set_flashdata('user_updated', '<p class=\'success\'>Products was successfully deleted.</p>');
						redirect("admincms/products/view_products");
					}
		} // end function update_products_arrange
				
			function edit_section()
		{	
			$data['site_title'] = "Edit Section";
			...
			$this->load->view('admincms/products/editSection', $data);

		} // end function edit_post
                
                function edit_products()
		{	
			$data['site_title'] = "Edit Products";
			...
			$this->load->view('admincms/products/editProducts', $data);
		...
		//	$this->form_validation->set_rules('desc_en', 'Section Desc. English', 'trim|xss_clean');
			...
			else
			{
				...
				$where_id='post_id';
				$data['rows'] = $this->data_model->editTable($table_name,$where_id,$id);	
				$data['update_record'] = "Products was successfully update";	
				$this->load->view('admincms/products/editProducts', $data);
				
			}
		} // end function update_products	
			...
			function update_section()
		{
			...
			{
				//echo "errors";
				$data['site_title'] = "Edit Services Section";
				$id = $this->input->post('section_id');
				$table_name='post_section';
				$where_id='section_id';
				$data['rows'] = $this->data_model->editTable($table_name,$where_id,$id);	
				$this->load->view('admincms/products/view_products', $data);
			...
			else
			{
                ...
                function update_services_order()
		{
				...
				$this->session->set_flashdata('user_updated', '<p class=\'success\'>Order was successfully updated.</p>');
				redirect("admincms/services/view_services");
		...
		{	
			...
			$data['site_title'] = "View Post";
                        ...
                        $data['post'] = $this->data_model->getProducts();
                        $this->load->view('admincms/products/view_products', $data);
