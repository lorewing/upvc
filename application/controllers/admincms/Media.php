...
//					   'group_desc_en' 	=> $this->input->post('group_desc_en'), 
		...
		function add_media()
		{
			...
			//Form Validation
			$this->form_validation->set_rules('title', 'Media Title Arabic', 'trim');
...
//							   'related_tag' 		=> $this->input->post('related_tag'),
		...
		{	
			...
			$data['site_title'] = "View Media Group";	
			$data['category'] = $this->data_model->getTable('media_group','group_id');
			$this->load->view('admincms/media/view_media_group', $data);
							...
							foreach($arrange as $group_id => $value2)
							  {
								  ...
								  $this->db->update('media_group', $data);
						 ...
						 foreach($contentid as $group_id => $value2)
						  {
							 	...
							 	$this->db->delete('media_group', array('group_id' => $value2)); 		
								 }
						$this->session->set_flashdata('user_updated', '<p class=\'success\'>Category was successfully deleted.</p>');
						redirect("admincms/media/view_media_group");
					}
		...
		function edit_media_group()
		{	
			$data['site_title'] = "Edit Categories";

			$id = $this->uri->segment(4,0);
			$table_name='media_group';
			$where_id='group_id';
			$data['rows'] = $this->data_model->editTable($table_name,$where_id,$id);
			
			$this->load->view('admincms/media/edit_media_group', $data);

		} // end function edit_category
		
		function update_media_group()
		{
